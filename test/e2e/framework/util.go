...
func WaitForPodsRunningReady(c clientset.Interface, ns string, minPods, allowedNotReadyPods int32, timeout time.Duration, ignoreLabels map[string]string) error {
	...
	if wait.PollImmediate(Poll, timeout, func() (bool, error) {
		...
		rsList, err := c.ExtensionsV1beta1().ReplicaSets(ns).List(ctx, metav1.ListOptions{})
		if err != nil {
...
func deleteNS(c clientset.Interface, dynamicClient dynamic.Interface, namespace string, timeout time.Duration) error {
	startTime := time.Now()
	ctx := context.TODO()
	if err := c.CoreV1().Namespaces().Delete(ctx, namespace, nil); err != nil {
		return err
	}
...
func getRuntimeObjectForKind(c clientset.Interface, kind schema.GroupKind, ns, name string) (runtime.Object, error) {
	...
	case extensionsinternal.Kind("ReplicaSet"), appsinternal.Kind("ReplicaSet"):
		return c.ExtensionsV1beta1().ReplicaSets(ns).Get(ctx, name, metav1.GetOptions{})
	case extensionsinternal.Kind("Deployment"), appsinternal.Kind("Deployment"):
		return c.ExtensionsV1beta1().Deployments(ns).Get(ctx, name, metav1.GetOptions{})
	case extensionsinternal.Kind("DaemonSet"):
		return c.ExtensionsV1beta1().DaemonSets(ns).Get(ctx, name, metav1.GetOptions{})
...
func UpdateDaemonSetWithRetries(c clientset.Interface, namespace, name string, applyUpdate updateDSFunc) (ds *apps.DaemonSet, err error) {
	...
	pollErr := wait.PollImmediate(10*time.Millisecond, 1*time.Minute, func() (bool, error) {
		...
		applyUpdate(ds)
		if ds, err = daemonsets.Update(ctx, ds); err == nil {
...
func CreateExecPodOrFail(client clientset.Interface, ns, generateName string, tweak func(*v1.Pod)) string {
	...
	ctx := context.TODO()
	created, err := client.CoreV1().Pods(ns).Create(ctx, execPod)
	Expect(err).NotTo(HaveOccurred())
	err = wait.PollImmediate(Poll, 5*time.Minute, func() (bool, error) {
		retrievedPod, err := client.CoreV1().Pods(execPod.Namespace).Get(ctx, created.Name, metav1.GetOptions{})
		if err != nil {
			if testutils.IsRetryableAPIError(err) {
				return false, nil
			}
...
func LaunchWebserverPod(f *Framework, podName, nodeName string) (ip string) {
	...
	createdPod, err := podClient.Get(ctx, podName, metav1.GetOptions{})
	ExpectNoError(err)
...
func UpdatePodWithRetries(client clientset.Interface, ns, name string, update func(*v1.Pod)) (*v1.Pod, error) {
	...
	for i := 0; i < 3; i++ {
		...
		update(pod)
		pod, err = client.CoreV1().Pods(ns).Update(ctx, pod)
		if err == nil {
			return pod, nil
		}
