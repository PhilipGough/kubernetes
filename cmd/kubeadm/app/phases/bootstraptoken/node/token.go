...
func UpdateOrCreateToken(client clientset.Interface, token string, failIfExists bool, tokenDuration time.Duration, usages []string, extraGroups []string, description string) error {
	...
	for i := 0; i < tokenCreateRetries; i++ {
		...
		if err == nil {
			...
			if _, err := client.CoreV1().Secrets(metav1.NamespaceSystem).Update(context.TODO(), secret); err == nil {
				return nil
			}
			lastErr = err
			continue
		...
		if apierrors.IsNotFound(err) {
			...
			if _, err := client.CoreV1().Secrets(metav1.NamespaceSystem).Create(context.TODO(), secret); err == nil {
				return nil
			}
			lastErr = err
			continue
